...
def decode_protobuf(encoded_data: bytes, message_type: message.Message) -> message.Message:
    ...
    # Fixed: Added check to prevent crash if data is corrupted
    try:
        instance.ParseFromString(encoded_data)
    except Exception as e:
        print(f"CRITICAL ERROR: Protobuf parsing failed: {e}")
    return instance
...
def get_account_credentials(region: str) -> str:
    ...
    if r == "ME":
        ...
        return "uid=4626850796&password=FLASH_TR_LQQRQCTWR"
...
async def create_jwt(region: str):
    try:
        ...
        if token_val == "0": return # Stop if login fails
        ...
        async with httpx.AsyncClient() as client:
            resp = await client.post(url, data=payload, headers=headers)
            # Check if server response is valid before decoding
            if resp.content and not resp.content.startswith(b'<html>'):
        ...
        print(f"Error in create_jwt for {region}: {e}")
